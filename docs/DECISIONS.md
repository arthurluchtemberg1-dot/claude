...
## D-028 — Leituras de métricas sem subconsultas por pedido; limites de ingestão configuráveis
- O teste de carga em contêineres mostrou o resumo de métricas com p50 ≈ 10 s e p95 ≈ 32 s para ~2.000 pedidos. Causa: a busca de toques pagos por crédito de atribuição era correlacionada (varredura de `touchpoints` por pedido, O(n²)). Correção: créditos expandidos uma vez (CTE materializada) e unidos a `touchpoints` em conjunto; resumo cai para ~70–100 ms com 2.000 pedidos (teste opcional `PERF=1` com limite de 500 ms).
- Os limites de ingestão por instância (webhooks por endpoint, coleta por IP) passam a ser configuráveis (`WEBHOOK_RATE_LIMIT_PER_MINUTE`, `COLLECT_RATE_LIMIT_PER_MINUTE`); os padrões (600 e 300/min) continuam conservadores e devem ser ajustados com base no teste de carga para picos de lançamento.
